...
def refine_prompt(state: State) -> State:
    ...
    response = llm.invoke(f"다음 문장을 이미지 생성용으로 개선하고, \
                          개선한 문장만 출력해줘: {user_input}")
...
def generate_image(state: State) -> State:
    ...
    return {"prompt": state["prompt"], "image_url": image_url}
...
workflow.set_exit_point("generate_image") # 종료 노드
